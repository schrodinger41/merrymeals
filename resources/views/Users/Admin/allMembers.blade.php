    ...
    <div class="container" style="margin-top: 120px; min-height: 60vh">
		...
		<div class="row">
				...
				<table class="table table-bordered">
					<thead>
						<tr>
							...
							<th scope="col" class="text-center" style="border: var(--primary-color) solid 1px; color: var(--primary-color);"> <h4> <b> Caregiver Name </b> </h4> </th>
          ...
          @foreach ($memberData as $member)
					<tbody>
							<tr>
								<td class="text-center" scope="row" style="border: var(--primary-color) solid 1px; color: var(--primary-color);">{{ DB::table('users')->where('id',$member->user_id)->value('name')}}</td>
								<td class="text-center" scope="row" style="border: var(--primary-color) solid 1px; color: var(--primary-color);">{{ $member->member_caregiver_name }}</td>
                ...
                <td class="text-center" scope="row" style="border: var(--primary-color) solid 1px; color: var(--primary-color);">{{ DB::table('users')->where('id',$member->user_id)->value('email')}}</td>
								<td class="text-center" scope="row" class="text-center" style="border: var(--primary-color) solid 1px; color: var(--primary-color);">
									...
									href="{{ route('admin#deleteMember', $member->user_id) }}"> <i class="far fa-trash-alt"></i>
								    </a></td>
							</tr>
					</tbody>
            @endforeach
